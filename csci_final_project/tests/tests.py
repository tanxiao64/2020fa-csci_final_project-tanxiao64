...
import pandas as pd
from csci_final_project.dataloader.filehandler import saveCSV, loadCSV, validate, file_exist
from csci_final_project.mockbacktest.backtest_output_producer import generate_fake_output
...
class FilehandlerTest(TestCase):
    def test_filehandler(self):
        ...
        with TemporaryDirectory() as tmp:
            filename = "asdf.csv"
            file_path = tmp + '/'
            tmp_data = pd.DataFrame([[1, 2, 3], [3, 4, 5]], index=[0, 1], columns=['start_date', 'end_date', 'pnl'])
...
class BacktestProducerTest(TestCase):
    def test_BacktestProducer(self):
        ...
        with TemporaryDirectory() as tmp:
            filename = "asdf.csv"
            file_path = tmp
            generate_fake_output(signal_num=5, row_num=10, filename=filename, file_path=file_path)
            loaded_data = loadCSV(filename, file_path)
    ...
    def testGeneticAlgorithm(self):
        with TemporaryDirectory() as tmp:
            ...
            tmp_data = pd.DataFrame([
                                     ...
                                     ], index=[0, 1, 2, 3, 4], columns=['start_date', 'end_date', 'pnl', 'signal_1', 'signal_2'])
